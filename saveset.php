...
if($_SERVER['HTTP_X_REQUESTED_WITH'] == "XMLHttpRequest" && isset($_POST['lis']) && ($_POST['del'] == "1" || (isset($_POST['tit']) && isset($_POST['pri']) && isset($_POST['sta']) && isset($_POST['pro']))))
{
	...
	if($_POST['del'] == "0")
	{
		...
		if($sql->exe($cfg->get("bugtracker"),"UPDATE `bt_message` SET `title` = '".$title."', `status` = '".$status."', `percentage` = '".$progress."', `priority` = '".$priority."' WHERE `id` = '".$listid."'"))
			echo 1;
		else
	...
	{
		if($sql->exe($cfg->get("bugtracker"),"DELETE FROM `bt_message` WHERE `id` = '".$_POST['lis']."'"))
			$m1 = 1;
		if($sql->exe($cfg->get("bugtracker"),"DELETE FROM `bt_options` WHERE `id` = '".$_POST['lis']."'"))
			$m2 = 1;
		if($sql->exe($cfg->get("bugtracker"),"DELETE FROM `bt_comment` WHERE `entry` = '".$_POST['lis']."'"))
			$m3 = 1;
		if($m1 == 1 && $m2 == 1 && $m3 == 1)
			echo 1;
		else
